...
//$get_mesg_query = "SELECT DISTINCT * FROM frei_chat WHERE frei_chat.to=" . $frm_id . "AND time<2 ORDER BY sent";
            ...
            $get_mesg_query = "SELECT * FROM frei_chat WHERE frei_chat.to=" . $frm_id . " AND time>" . $time . " AND frei_chat.message_type!=1 ORDER BY sent ";
...
//for($i=0;$i<=10;$i++){
        foreach ($result as $res) {
            ...
            if ($this->linkprofile != 'disabled' && $res['guest'] == 0 && $_SESSION[$this->uid . "is_guest"] == 0) {
...
//-------------------------------------------------------------------
    public function post_mesg() {
        ...
        if ($message_type == 2) {
            ...
            require_once 'video/video_api.php';

            $video_api = new video_api($this->db);
            $video_api->from = $this->frm_id;
            $video_api->to = $to;

            $is_video_session_created = $video_api->is_video_session_created();
            if ($is_video_session_created == false) {
                $video_api->create_video_session($_SESSION[$this->uid . 'gst_ses_id']);
                $video_api->add_video_session();
            }

            $freichat->token = $video_api->token;
            $freichat->sessionId = $video_api->sessionId;
        }
...
//-------------------------------------------------------------------------
    public function get_data() {
        ...
        $getdata_query = "SELECT * FROM frei_chat WHERE (frei_chat.to=" . $frm_id . " OR frei_chat.from=" . $frm_id . ") AND message_type<>1 AND message_type<>2 ORDER BY sent";
...
//-------------------------------------------------------------------------------
    public function isset_msg() {
        ...
        $isset_mesg_query = "SELECT * FROM frei_chat WHERE ((frei_chat.to=" . $frm_id . " AND frei_chat.from=" . $this->db->quote($to_id) . ") OR (frei_chat.from=" . $frm_id . " AND frei_chat.to=" . $this->db->quote($to_id) . ") ) AND frei_chat.message_type <> 2 ORDER BY sent";
