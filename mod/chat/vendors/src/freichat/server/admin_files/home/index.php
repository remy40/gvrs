...
class param extends FC_admin {
    ...
    public function purge_mesg_history($days) {
        $time = $days * 24 * 60 * 60;
        $delete_mesg_query = "DELETE FROM frei_chat  WHERE rec =1 AND sent < NOW()-" . $time;
        global $db;
        $db->query($delete_mesg_query);
    }

    public function create_url($name) {
        $url = str_replace("server/admin.php", "", $this->url);

        return $url . 'client/jquery/freichat_themes/' . $this->freichat_theme . '/' . $name;
    }
...
<script type="text/javascript">
    ...
    function add_smiley_row(symbol,name) {
        ...
        var url = '<?php echo str_replace("server/admin.php", "", $param->url); ?>';
        
        url =  url+'client/jquery/freichat_themes/<?php echo $param->freichat_theme; ?>/'+name;
        var image = "<img id="+name+" src="+url+" />";
        var tr = "<tr id='smiley_tr_"+id+"'><td>"+id+"</td><td id='smiley_symbol_"+id+"'>"+symbol+"</td><td>"+image+"</td><td><img height=12px width=12px onmousedown=delete_tr("+id+",'"+name+"') src='admin_files/theme_maker/delete.png' /></td></tr>";
