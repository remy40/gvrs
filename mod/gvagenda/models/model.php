<?php
    require_once(elgg_get_plugins_path() . "gvagenda/models/frequency.php");
/**
 * Elgg event model
 *
...
/**
 * when an event is repeatitive, create all child events.
 * @param $event the parent event
 * @return an array of child events
 */
function gvagenda_create_child_events($event) {
    $duration = $event->repeat_duration;
    $frequency = $event->repeat_frequency;
    $childevents = array();
    $dateinterval = gvagenda_get_dateinterval($frequency);

    if ($dateinterval) {

        // get start and end date in DateTime objects
        $startdate = new DateTime;
        $startdate->setTimestamp($event->start_date);
        
        if ($event->end_date) {
            $enddate = new DateTime;
            $enddate->setTimestamp($event->end_date);
        }
        ...
        for ($i = 1; $i <= $duration; $i++) {
            ...
            $childevent->start_date = $startdate->getTimestamp();
            
            if ($enddate) {
                $enddate->add($dateinterval);
                $childevent->end_date = $enddate->getTimestamp();
            }
        ...
        foreach($childevents as $childevent) {
            $childevent->addRelationship($event->guid, 'child_event');
        }
    }
    else {
        return false;
 ...
 * 
 */
function gvagenda_delete_child_events($event) {
    if ($event) {
        $children = $event->getEntitiesFromRelationship('child_event', true);
 ...
 * 
 */
function gvagenda_update_child_events($event) {
    if ($event) {
        $children = $event->getEntitiesFromRelationship('child_event', true);
        $all_fields = array('title','venue','start_time','end_time','end_date',
            'description','fees','contact','organiser',
            'long_description','repeat_event', 'repeat_frequency', 'repeat_duration');
        
        foreach($children as $child) {
            foreach($all_fields as $field) {
                $child->$field = $event->$field;
            }

            $child->access_id = $event->access_id;
            $child->save();
        }
    }
}
...
*/
    $newevent = ($event_guid == 0);
    $required_fields = array('title','venue','start_date', 'end_date',
    ...
    error_log("end_date from picker : ".date("Y-m-d", $end_date));
	if ($end_date) {
    ...
    $repeat_event_received = get_input('repeat_event');
    if (is_array($repeat_event_received)) {
        $repeat_event = in_array('yes', $repeat_event_received);
    }
    else {
        $repeat_event = ($repeat_event_received == 'yes');
    }
    
    $repeat_frequency = get_input('repeat_frequency');
    $repeat_duration = get_input('repeat_duration');
    $repeat_changed = ($event->repeat_event != $repeat_event);
    $repeat_params_changed = (($event->repeat_frequency != $repeat_frequency) || 
                              ($event->repeat_duration != $repeat_duration) ||
                              ($event->start_date != $start_date) ||
                              ($event->end_date && ($event->end_date != $end_date)));
    ...
    // now, we know if repeat params have changed, so we can update the event
    $event->repeat_event = get_input('repeat_event');
    $event->repeat_frequency = get_input('repeat_frequency');
    $event->repeat_duration = get_input('repeat_duration');
    ...
    if ($missing_fields) {
        $error_string = "";
        foreach($missing_field_list as $field) {
            $error_string .= elgg_echo("gvagenda:$field").",";
        }

        $error_string = rtrim($error_string, ",");
        register_error(elgg_echo('gvagenda:missing_fields',array($error_string)));
        return false;
    }

    if (($event->start_time == '-') || ($event->end_time == '-')) {
        register_error(elgg_echo("gvagenda:time_error"));
        return false;
    }

    // check repeat duration
    if ($event->repeat_event && !is_numeric($event->repeat_duration)) {
        register_error(elgg_echo("gvagenda:repeat_duration_error"));
        return false;
    }

    // date consistency
    $start_date_without_time = strtotime(date("Y-m-d",$event->start_date)." ".date_default_timezone_get());

    // start date must be later than now
    if ($start_date_without_time < strtotime(date("Y-m-d")." ".date_default_timezone_get())) {
        register_error(elgg_echo("gvagenda:past_date_error"));
        return false;
    }

    if (($event->end_date - $start_date_without_time) < 0) {
        register_error(elgg_echo("gvagenda:date_error"));
        return false;
    }
    else if (($event->end_date == $start_date_without_time) && ($event->end_time - $event->start_time) < 0) {
        register_error(elgg_echo("gvagenda:time_error"));
        return false;
    }
	...
	if ($event->save()) {
        if ($newevent) {
            // if it's a repeatitive event, create child events
            if ($event->repeat_event) {
                $childevents = gvagenda_create_child_events($event);
            }
        }
        else {
            if ($repeat_changed) {
                if ($event->repeat_event) {
                    ...
                    gvagenda_delete_child_events($event);
                }
            }
            else {
                if ($event->repeat_event) {
                    if ($repeat_params_changed) {
                        // @todo: here, the best is to create only new child events
                        gvagenda_delete_child_events($event);
                        $childevents = gvagenda_create_child_events($event);
                    }
                    else {
                        gvagenda_update_child_events($event);
                    }
                }
            }
        }
