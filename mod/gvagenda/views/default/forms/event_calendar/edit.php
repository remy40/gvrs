...
$repeat_frequency_options = gvagenda_get_frequency_options();
...
$required_fields = array('title','venue','start_date', 'end_date',
        'start_time', 'end_time', 'brief_description','organiser');
...
foreach ($all_fields as $fn) {
	if (in_array($fn,$required_fields)) {
		$suffix[$fn] = "<span class=\"required-field\"> *</span>";
	} 
}
...
if ($event) {
    ...
    $repeat_duration = $event->repeat_duration;
...
$body .= '<p><label>'.elgg_echo("event_calendar:title_label").'</label>'.$suffix['title'].'<br/>';
$body .= elgg_view("input/text",array('name' => 'title','value'=>$title));
$body .= '</p>';

$body .= '<p><label>'.elgg_echo("event_calendar:venue_label").$suffix['venue'].'<br />';
...
if ($event_calendar_times != 'no') {
	...
	if ($event_calendar_hide_end != 'yes') {
		$body .= '<p><label>'.elgg_echo('event_calendar:from_label').$suffix['start_date'].'</label>';
	}
	...
	if ($event_calendar_hide_end != 'yes') {
		$body .= '</p><p><label>'.elgg_echo('event_calendar:to_label').$suffix['end_date'].'</label>';
	...
	$body .= '<p><label>'.elgg_echo("event_calendar:start_date_label").$suffix['start_date'].'</label><br />';
...
//	$body .= '<p class="description">'.$prefix['start_date'].elgg_echo('event_calendar:start_date_description').'</p>';
	...
	if ($event_calendar_hide_end != 'yes') {		
		$body .= '<p><label>'.elgg_echo("event_calendar:end_date_label").$suffix['end_date'].'</label><br />';
...
$body .= '<p>';
$body .= elgg_view("input/checkboxes",array('name' => 'repeat_event','options' => array(elgg_echo("gvagenda:repeat_event_label") => 'yes'), 'value' => ($repeat_event?'yes':'no')));
$body .= '</p>';

$body .= '<p><label>'." ".elgg_echo("gvagenda:repeat_frequency_label").$suffix['repeat_frequency'].'</label>';
$body .= elgg_view("input/dropdown",array('name' => 'repeat_frequency','value'=>$repeat_frequency, 'options_values' => $repeat_frequency_options));
$body .= '</p>';

$body .= '<p><label>'.elgg_echo("gvagenda:repeat_duration_label").$suffix['repeat_duration'].'</label>';
$body .= elgg_view("input/text",array('name' => 'repeat_duration','value'=>$repeat_duration));
$body .= '</p>';

if ($event_calendar_spots_display == 'yes') {
	$body .= '<p><label>'.elgg_echo("event_calendar:spots_label").$suffix['spots'].'<br />';
...
$body .= '<p><label>'.elgg_echo("event_calendar:brief_description_label").$suffix['brief_description'].'<br />';
...
if ($event_calendar_region_display == 'yes') {
	...
	if ($region_list) {
		...
		$body .= '<p><label>'.elgg_echo("event_calendar:region_label").$suffix['region'].'<br />';
...
if ($event_calendar_type_display == 'yes') {
	...
	if ($type_list) {
		...
		$body .= '<p><label>'.elgg_echo("event_calendar:type_label").$suffix['event_type'].'<br />';
...
$body .= '<p><label>'.elgg_echo("event_calendar:fees_label").$suffix['fees'].'<br />';
...
$body .= '<p><label>'.elgg_echo("event_calendar:contact_label").$suffix['contact'].'<br />';
...
$body .= '<p><label>'.elgg_echo("event_calendar:organiser_label").$suffix['organiser'].'<br />';
...
/*
$body .= '<p><label>'.elgg_echo("event_calendar:event_tags_label").$suffix['tags'].'<br />';
...
$body .= '<p><label>'.elgg_echo("event_calendar:long_description_label").$suffix['long_description'].'<br />';
