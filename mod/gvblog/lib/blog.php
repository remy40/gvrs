...
/**
 * extract only sitewide blog posts (those one that have not container_guid which is a group)
 */
function elgg_get_sitewide_blog($options) {
...
function blog_get_page_content_list($container_guid = NULL) {
    ...
    if ($container_guid) {
    	...
    	$list = elgg_list_entities($options, elgg_get_sitewide_blog);
    }
	if (!$list) {
